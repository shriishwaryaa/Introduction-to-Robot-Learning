...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.standardize_advantages:
            ## TODO: standardize run_baseline_predictionthe advantages to have a mean of zero
    ...
    def _discounted_return(self, rewards):
        ...
        gamma_array = np.array([self.gamma**i for i in range(len(rewards))])
        discounted_returns = np.dot(gamma_array, np.array(rewards))
        discounted_returns = np.repeat(discounted_returns, len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        gamma_array = np.array([self.gamma**i for i in range(T)])
        rewards_array = np.array(rewards)

        for i in range(T):
            reward_to_go = np.dot(gamma_array[:T-i], rewards_array[i:])
            discounted_cumsums[i] = reward_to_go
